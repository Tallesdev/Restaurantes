package Restaurante.example.Restaurante.model;

import jakarta.persistence.Entity;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.Table;

@Entity // Indica que esta classe é uma entidade JPA e mapeia uma tabela
@Table(name = "restaurantes") // Define o nome da tabela no BD
public class Restaurante {

    @Id // Define o campo como Chave Primária
    @GeneratedValue(strategy = GenerationType.IDENTITY) // Configura auto-incremento
    private Long id;

    // Nome (Requisito: Nome da Tabela inicial)
    private String nome;

    // Endereço (Campo sugerido)
    private String endereco;
    
    // Telefone (Campo sugerido)
    private String telefone;
    
    // Construtor padrão (obrigatório pelo JPA)
    public Restaurante() {
    }

    // Construtor com campos (opcional, mas útil)
    public Restaurante(String nome, String endereco, String telefone) {
        this.nome = nome;
        this.endereco = endereco;
        this.telefone = telefone;
    }

    // --- Getters e Setters (São cruciais para o JPA e Thymeleaf) ---

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getNome() {
        return nome;
    }

    public void setNome(String nome) {
        this.nome = nome;
    }

    public String getEndereco() {
        return endereco;
    }

    public void setEndereco(String endereco) {
        this.endereco = endereco;
    }

    public String getTelefone() {
        return telefone;
    }

    public void setTelefone(String telefone) {
        this.telefone = telefone;
    }
}